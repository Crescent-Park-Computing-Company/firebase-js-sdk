...
from the planned range sizes instead of a second full-tree walk; the identity
diff bounds its own work (visit budget) so a divorced baseline collapses to
all-dirty in milliseconds instead of a double-tree walk;
