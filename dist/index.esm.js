...
const version = "1.1.3-persistence-v2.6";
                    ...
                    for (let i = 0; i < keys.length; i++) {
                        ...
                        if (rest === '') {
                            ...
                            for (let j = 0; j < path.length; j++) {
                                path[j] = path[j].replace(/%([0-9a-f]{4})/g, (_m, hex) => String.fromCharCode(parseInt(hex, 16)));
                            }
                        }
...
function decodeRowKeyRelativePath(key, scope, rootString) {
    const prefix = encodeRowSegment(scope) +
        ROW_KEY_SEPARATOR +
        encodeRowSegment(rootString) +
        ROW_KEY_SEPARATOR;
...
class RowPersistenceManager {
    ...
    async flushWholeRoot_(pathString, root, node, generation) {
        ...
        const ownsMarker = (txn) => this.requestDone_(txn.objectStore(META_STORE).get(metaKey)).then(meta => meta !== undefined &&
            meta.staging === true &&
            meta.gen === stageNonce);
