...
class PersistentConnection extends ServerActions {
    ...
    listen(query, currentHashFn, tag, onComplete, onProgress, onResend) {
        this.initConnection_();
...
class ReadonlyRestClient extends ServerActions {
    ...
    /** @inheritDoc */
    listen(query, currentHashFn, tag, onComplete, onProgress, _onResend) {
        const pathString = query._path.toString();
 ...
 */
function encodeRowSegment(segment) {
...
const ROW_PERSISTENCE_WRITE_DEBOUNCE_MS = 30000;
...
const ROW_PERSISTENCE_ORPHAN_GEN_AGE_MS = 10 * 60 * 1000;
/** Serialized bytes per chunk — the slice unit for parse and write. */
const ROW_PERSISTENCE_CHUNK_BYTES = 2 * 1024 * 1024;
...
class RowPersistenceManager {
    ...
    async acquireWriterLock_(pathString, root) {
        ...
        let decided;
        root.lockDecided = new Promise(resolve => {
            decided = resolve;
        });
        try {
            let probeWon = false;
            await this.webLocks_.request(name, { mode: 'exclusive', ifAvailable: true }, lock => {
                decided();
                if (lock === null || !usable()) {
                    return Promise.resolve();
                }
    ...
    metaKey_(pathString) {
        return (encodeRowSegment(this.scopeKey_()) +
            ROW_KEY_SEPARATOR +
            encodeRowSegment(pathString) +
            ROW_KEY_SEPARATOR);
    }
    /** chunk key: metaKey · gen · index (zero-padded for range order). */
    chunkKey_(pathString, gen, index) {
        return (this.metaKey_(pathString) +
            gen +
            ROW_KEY_SEPARATOR +
            String(index).padStart(6, '0'));
    }
    chunkRange_(pathString, gen) {
        const start = gen === undefined
    ...
    async readRoot_(pathString) {
        ...
        try {
            ...
            const meta = (await this.requestDone_(metaReq));
            if (meta === undefined ||
            ...
            const chunkTexts = (await this.requestDone_(chunksReq));
            ...
            for (let i = 0; i < chunkTexts.length; i++) {
                const parsed = JSON.parse(chunkTexts[i]);
                for (let j = 0; j < parsed.length; j++) {
                    rows.push(parsed[j]);
                }
    ...
    async computeListenHashesOnMainThread_(pathString, gen) {
        const db = await this.open_();
        if (db === null) {
            return null;
        }
        try {
            ...
            if (meta === undefined ||
                meta.gen !== gen ||
                chunkTexts.length !== meta.chunkCount ||
                chunkTexts.length === 0) {
                return null;
            }
            const rows = [];
     ...
     */
    async flushImpl_(pathString, root) {
            ...
            if (node.isLeafNode() || node.isEmpty()) {
                ...
                for (const [key, child] of children) {
                    ...
                    if (payload === undefined) {
                        payload = JSON.stringify(splitNodeIntoRows([key], child, this.splitThresholdBytes_));
                        childPayloadCache.set(child, payload);
                        await yieldMacrotask();
                    }
                    pushRows(JSON.parse(payload));
                }
            }
        ...
        try {
            ...
            for (let i = 0; i < metaKeys.length; i++) {
                ...
                if (meta === null ||
                    ...
                    metaStore.delete(metaKeys[i]);
                    deletions++;
                }
                else {
                    live.set(metaKeys[i], meta.gen);
                }
            }
    ...
    rebindTo(prefix) {
        const rebound = new RowPersistenceManager(prefix, this.idbFactory_, this.webLocks_, this.writeDelayMs_, this.firstGenDelayMs_, this.splitThresholdBytes_, this.peekHandoffMs_, this.peekPreAuthMs_, this.restoreTimeoutMs_, this.chunkBytes_, this.workerHashTimeoutMs_, this.maxAgeMs_, this.orphanGenAgeMs_);
