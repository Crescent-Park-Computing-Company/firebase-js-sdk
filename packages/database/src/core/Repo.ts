...
import { markDirtyRanges, StableRange } from './CompoundHash';
import {
...
export function repoStartServerListen(
   ...
   */
  let grafts: Array<{ path: Path; complete: Node }> | null = null;
  const collectGraftableDescendants = (): typeof grafts => {
   ...
   */
  const blankGraftedRangeHashes = (
    compoundHash: SeedCompoundHash,
    graftPaths: string[][]
  ): SeedCompoundHash => {
    const ranges: StableRange[] = compoundHash.posts.map((post, index) => ({
      post,
    ...
    if (grafts.length > 0) {
      ...
      if (pending !== undefined) {
        ...
        repo.pendingListenHashes_.set(
          pathString,
          '',
          blankGraftedRangeHashes(
            pending.compoundHash,
            ...
            graftList.length === 0
              ...
              : stampSeedHashes(
                  ...
                  record.compoundHash !== undefined
                    ? blankGraftedRangeHashes(
                        record.compoundHash,
                        graftList.map(graft => pathSlice(graft.path))
                      )
                    : undefined
                );
